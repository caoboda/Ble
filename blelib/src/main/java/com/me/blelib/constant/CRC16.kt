...
internal object CRC16 {
    ...
    fun CCITT_Kermit(data: ByteArray, offset: Int, length: Int): Int {
        return CRC(0x1021, 0x0000, data, offset, length, refin = true, refout = true, xorout = 0x0000)
    }
    ...
    fun CCITT_FALSE(data: ByteArray, offset: Int, length: Int): Int {
        ...
        return CRC(0x1021, 0xFFFF, data, offset, length, refin = false, refout = false, xorout = 0x0000)
    }
    ...
    fun MCRF4XX(data: ByteArray, offset: Int, length: Int): Int {
        return CRC(0x1021, 0xFFFF, data, offset, length, refin = true, refout = true, xorout = 0x0000)
    }
    ...
    fun AUG_CCITT(data: ByteArray, offset: Int, length: Int): Int {
        return CRC(0x1021, 0x1D0F, data, offset, length, refin = false, refout = false, xorout = 0x0000)
    }
    ...
    fun ARC(data: ByteArray, offset: Int, length: Int): Int {
        return CRC(0x8005, 0x0000, data, offset, length, refin = true, refout = true, xorout = 0x0000)
    }
    ...
    fun MAXIM(data: ByteArray, offset: Int, length: Int): Int {
        return CRC(0x8005, 0x0000, data, offset, length, refin = true, refout = true, xorout = 0xFFFF)
    }

    /**
     * Calculates CRC-16 MODBUS over given range of bytes from the block of data.
     * It is using the 0x8005 polynomial and 0xFFFF initial value and XORs output with 0x0000.
     *
     *
     * Input data and output CRC are reversed.
     *
     *
     * See: http://reveng.sourceforge.net/crc-catalogue/16.htm#crc.cat.crc-16-modbus
     *
     * @param data   The input data block for computation.
     * @param offset Offset from where the range starts.
     * @param length Length of the range in bytes.
     ...
     */
    fun MODBUS(data: ByteArray, offset: Int, length: Int): Int {
        return CRC(0x8005, 0xFFFF, data, offset, length, refin = true, refout = true, xorout = 0x0000)
    }

    /**
     * Calculates CRC-16 XModem over given range of bytes from the block of data.
     * It is using the 0x1021 polynomial and 0x0000 initial value and XORs output with 0x0000.
     *
     * See: http://reveng.sourceforge.net/crc-catalogue/16.htm#crc.cat.crc-16-xmodem
     *
     * @param data   The input data block for computation.
     * @param offset Offset from where the range starts.
     * @param length Length of the range in bytes.
     * @return the CRC-16 XModem.
     */
    fun XMODEM(data: ByteArray, offset: Int, length: Int): Int {
        return CRC(0x1021, 0x0000, data, offset, length, refin = false, refout = false, xorout = 0x0000)
    }
    ...
    fun CRC(
        poly: Int,
        init: Int, @NonNull data: ByteArray,
        offset: Int,
        length: Int,
        refin: Boolean,
        refout: Boolean,
        ...
        while (i < offset + length && i < data.size) {
            ...
            for (j in 0..7) {
                ...
                val bit = ((b.toInt() and 0xFF) shr (7 - k) and 1) == 1
